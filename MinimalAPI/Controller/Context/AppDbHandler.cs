...
namespace MinimalAPI.Controller.Context
{
    public class AppDbHandler
    {
        ...
        public AppDbHandler(WebApplication app)
        {
            ...
            // Отель по id
            app.MapGet("/hotels/{id}", async (IHotelRepository repository, int id) => await repository.GetHotelByIdAsync(id))
            ...
            // Отель по названию
            app.MapGet("/hotels/search/name/{query}", async (IHotelRepository repository, string query) => await repository.GetHotelsByName(query))
                .Produces<List<Hotel>>(StatusCodes.Status200OK)
